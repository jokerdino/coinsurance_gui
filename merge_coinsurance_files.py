...
def merge_multiple_files(list_file_names, new_file_name):
    ...
    if bool_claims and bool_premium:
        with pd.ExcelWriter(f"{new_file_name}.xlsx", datetime_format="dd/mm/yyyy") as writer:
            output_claims.sort_values('Voucher date').to_excel(writer, index=False, sheet_name="CR")
            output_premium.sort_values('Accounting date').to_excel(writer, index=False, sheet_name="PP")
