...
def merge_files(df_pp, df_claims_reports, df_claims_data, path_string_wip, bool_hub):
    ...
    df_claims_receivable = generate_claim_reports(df_claims_reports, df_claims_data, bool_hub)
    ...
    list_premium_payable_coinsurers = df_premium_payable["Name of coinsurer"].unique().tolist()

    list_concatenated = list(set(list_claims_coinsurers).intersection(list_premium_payable_coinsurers))
    list_claims_alone = list(set(list_claims_coinsurers) - set(list_premium_payable_coinsurers))
    list_premium_alone = list(set(list_premium_payable_coinsurers) - set(list_claims_coinsurers))
    print(f"{list_claims_alone=}")
    print(f"{list_premium_alone=}")
    print(f"{list_concatenated=}")#unique_list_concatenated =
    ...
    for company in sorted(list_claims_alone):
        ...
        df_company_claims = df_claims_receivable[df_claims_receivable["Name of coinsurer"].str.contains(company)]

        with pd.ExcelWriter(f"{path_string_wip}/{company}/{company}.xlsx", datetime_format="dd/mm/yyyy") as writer:
            df_company_claims.to_excel(writer, index=False,sheet_name="CR")
            worksheet_formatter(writer, "CR")

        generate_pivot_table(f"{path_string_wip}/{company}/{company}.xlsx",company, path_string_wip)

    for company in sorted(list_premium_alone):
        if not os.path.exists(f"{path_string_wip}/{company}"):
            os.mkdir(f"{path_string_wip}/{company}")
        df_company_premium = df_premium_payable[df_premium_payable["Name of coinsurer"].str.contains(company)]

        with pd.ExcelWriter(f"{path_string_wip}/{company}/{company}.xlsx", datetime_format="dd/mm/yyyy") as writer:
            df_company_premium.to_excel(writer, index=False, sheet_name="PP")
